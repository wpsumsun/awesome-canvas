class Scratchcard {
   constructor ({
     ...
     this.awardBackgroundImage = awardBackgroundImage;
     ...
     this.drawAwardBackgroundImage(this.canvas);
  ...
  drawAwardBackgroundImage(canvas) {
    canvas.setAttribute(
      'style',
      `background: url(${this.awardBackgroundImage}) no-repeat center center;`
    )
  }
  ...
  drawEraser(ctx, loc) {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.arc(loc.x, loc.y, this.eraserSize, 0, Math.PI * 2, false);
    ctx.clip();
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ctx.restore();
    const percentage =  this.getFilledPercentage(ctx, this.canvas);
    if (percentage >= 50) {
      this.isDone = true;
  ...
  bindEvent(canvas) {
    ...
    ['touchmove', 'mousemove'].forEach((event) => {
      canvas.addEventListener(event, (e) => {
        let loc;
        if (this._dragging) {
          loc = this.windowToCanvas(canvas, e);
          this.drawEraser(this.ctx, loc);
        }
      })
    });
